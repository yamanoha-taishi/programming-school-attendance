...
| --- | --- | --- | --- | --- |
| id | integer | 不可 | PK | ID |
| lesson_plan_id | integer | 不可 | FK → lesson_plans.id | 授業内容 |
| section_id | integer | 不可 | FK → sections.id | 授業実施単位 |
| date | date | 不可 |  | 日付 |
| created_at | timestamp | 不可 |  | 作成日時 |
| updated_at | timestamp | 不可 |  | 更新日時 |
...
複合UNIQUE制約：(lesson_plan_id, section_id)
...
erDiagram
  ...
  LESSONS["LESSONS:授業回"] {}
  LESSON_PLANS["LESSON_PLANS:授業内容"] {}
  ...
  SCHOOL_CLASSES ||--o{ LESSON_PLANS : "has"
  LESSON_PLANS ||--o{ LESSONS : "has"
